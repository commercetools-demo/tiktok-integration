...
export const initializeShop = async (
  ...
  const warehouseChannels = await ChannelController.findChannel(apiRoot, ['custom(fields(isTikTokWarehouse=true))']);
  ...
  const priceChannels = await ChannelController.findChannel(apiRoot, ['custom(fields(isTikTokPrice=true))']);
