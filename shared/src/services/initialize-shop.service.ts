...
export const initializeShop = async (
  ...
  const isAuthorized =
    await CommercetoolsStorage.ShopConfigController.isAuthorized(
      apiRoot,
      app_key,
    );
  if (!isAuthorized) {
  ...
  const warehouse = warehouses.find(
    (warehouse) =>
      warehouse.id === ctWarehouseChannel?.custom?.fields.warehouseId,
  );
  if (!warehouse) {
  ...
  await CommercetoolsStorage.CategoryController.storeCategories(
    apiRoot,
    app_key,
    categories,
  );
